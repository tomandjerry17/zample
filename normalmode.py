    ...
    def timed_game(self):
        ...
        def display_next_question():
            ...
            if time.time() < end_time:
                random_morse, correct_answer = morse_codes_copy.pop(0)
                self.display_question(random_morse)
                ...
                submit_button = tk.Button(self.current_game_window, text="Submit",
                                          command=lambda r=random_morse, c=correct_answer: self.check_answer(user_input,
                                                                                                             r, c),
                                          font=("Helvetica", 14))
                submit_button.grid(row=3, column=0, pady=10)

                self.current_game_window.after(1000, display_next_question)
            else:
                self.display_score()
